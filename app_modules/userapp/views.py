...
def index_view(request):
    ...
    all_toys = models.toy.objects.all().order_by('?')[:4]  # Randomly select 4 toys for the homepage]
    new_arrivals = models.toy.objects.all().order_by('-created_at')
    popular_toys = models.toy.objects.all().order_by('?')# Adjust logic if you have sales count
    featured_toys = models.toy.objects.filter(is_available=True).order_by('?')
...
def wishlist_add(request, id):
    ...
    product = models.toy.objects.get(id=id)
